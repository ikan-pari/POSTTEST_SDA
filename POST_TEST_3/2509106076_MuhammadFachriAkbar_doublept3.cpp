...
void menuDequeue(Queue &q, Stack &s, Stack &redoStack) {
    if (isQueueEmpty(q)) {
...
void menuUndo(Stack &riwayat, Stack &redoStack) {
    cout << "\n  ===== Undo Transaksi Terakhir =====\n";
    garisPendek();

    if (isStackEmpty(riwayat)) {
        cout << "  [UNDERFLOW] Tidak ada transaksi yang bisa di-undo.\n";
        return;
    }
    if (isStackFull(redoStack)) {
        cout << "  [OVERFLOW] Stack redo penuh! Tidak bisa menyimpan undo.\n";
        return;
    }
    ...
    cout << "  Transaksi berhasil di-undo:\n";
    headerTabelPenumpang();
    cetakPenumpang(1, &p);
    garisPendek();
    cout << "  Transaksi atas nama " << p.nama << " dipindahkan ke redo stack.\n";
    cout << "  Sisa riwayat  : " << (riwayat.top + 1) << " transaksi.\n";
    cout << "  Redo tersedia : " << (redoStack.top + 1) << " transaksi.\n";
}

void menuRedo(Stack &riwayat, Stack &redoStack) {
    cout << "\n  ===== Redo Transaksi =====\n";
    garisPendek();

    if (isStackEmpty(redoStack)) {
        cout << "  [UNDERFLOW] Tidak ada transaksi yang bisa di-redo.\n";
        return;
    }
    if (isStackFull(riwayat)) {
        cout << "  [OVERFLOW] Stack riwayat penuh! Tidak bisa redo.\n";
        return;
    }
    ...
    cout << "  Transaksi berhasil di-redo:\n";
    headerTabelPenumpang();
    cetakPenumpang(1, &p);
    garisPendek();
    cout << "  Transaksi atas nama " << p.nama << " dikembalikan ke riwayat.\n";
    cout << "  Total riwayat : " << (riwayat.top + 1) << " transaksi.\n";
    cout << "  Redo tersisa  : " << (redoStack.top + 1) << " transaksi.\n";
}
...
int main() {
    ...
    Queue antrian;
    Stack riwayat;
    Stack redoStack;       // Stack khusus untuk redo
    initQueue(antrian);
    initStack(riwayat);
    initStack(redoStack);
    ...
    do {
        ...
        cout << "  ---- Undo & Redo ----\n";
        cout << "  15. Undo Transaksi Terakhir\n";
        cout << "  16. Redo Transaksi\n";
        cout << "  0.  Keluar\n";
        garis();
        ...
        switch (pilihan) {
            ...
            case 10: menuDequeue(antrian, riwayat, redoStack); break;
            ...
            case 15: menuUndo(riwayat, redoStack); break;
            case 16: menuRedo(riwayat, redoStack); break;
