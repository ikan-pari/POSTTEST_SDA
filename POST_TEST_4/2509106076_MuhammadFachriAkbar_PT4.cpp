...
NodePenumpang* buatNode(const string &nama, int noKA,
                        const string &asal, const string &tujuan,
                        double harga) {
    NodePenumpang* node = new NodePenumpang;
    node->nama        = nama;
    node->nomorKereta = noKA;
    node->asalKota    = asal;
    node->tujuanKota  = tujuan;
    node->hargaTiket  = harga;
    node->next        = nullptr;
    return node;
}

void enqueue(Queue &q, const string &nama, int noKA,
             const string &asal, const string &tujuan, double harga) {
    NodePenumpang* node = buatNode(nama, noKA, asal, tujuan, harga);
    if (isQueueEmpty(q)) {
        q.front = node;
        q.rear  = node;
    } else {
...
bool dequeue(Queue &q, NodePenumpang &out) {
    if (isQueueEmpty(q)) return false;
    NodePenumpang* tmp = q.front;
    out = *tmp;
    out.next = nullptr;
...
bool peekQueue(const Queue &q, NodePenumpang* &out) {
    if (isQueueEmpty(q)) return false;
    out = q.front;
    return true;
}
...
void push(Stack &s, const NodePenumpang &data) {
    NodePenumpang* node = buatNode(data.nama, data.nomorKereta,
                                   data.asalKota, data.tujuanKota,
                                   data.hargaTiket);
    node->next = s.top;
    s.top      = node;
}

bool pop(Stack &s, NodePenumpang &out) {
    if (isStackEmpty(s)) return false;
    NodePenumpang* tmp = s.top;
    out      = *tmp;
    out.next = nullptr;
    s.top    = s.top->next;
    delete tmp;
...
bool peekStack(const Stack &s, NodePenumpang* &out) {
    if (isStackEmpty(s)) return false;
    out = s.top;
    return true;
}
...
void cetakNode(int no, const NodePenumpang *p) {
    cout << left
         << setw(4)  << no
         << setw(20) << p->nama
...
void menuEnqueue(Queue &q, Kereta *arr, int n) {
    ...
    enqueue(q, nama, k->nomorKereta, k->asalKota, k->tujuanKota, k->hargaTiket);
    int posisi = 1;
    NodePenumpang* cur = q.front;
    while (cur->next != nullptr) { posisi++; cur = cur->next; }
    cout << "\n  " << nama << " berhasil masuk antrian untuk kereta "
         << k->namaKereta << " (" << k->asalKota << " -> " << k->tujuanKota << ").\n";
    cout << "  Posisi dalam antrian: " << posisi << "\n";
}
...
void menuDequeue(Queue &q, Stack &s) {
    ...
    cetakNode(1, &keluar);
    garisPendek();
    cout << "  Tiket berhasil diterbitkan untuk " << keluar.nama << "!\n";
    push(s, keluar);
...
void menuPop(Stack &s) {
    ...
    cetakNode(1, &keluar);
    garisPendek();
    cout << "  Transaksi atas nama " << keluar.nama << " telah dihapus dari riwayat.\n";
}
...
void menuPeek(const Queue &q, const Stack &s) {
    ...
    if (peekQueue(q, ptr)) { headerTabelPenumpang(); cetakNode(1, ptr); }
    else cout << "  Antrian kosong.\n";
    cout << endl;
    cout << "  [RIWAYAT] Transaksi terakhir:\n";
    if (peekStack(s, ptr)) { headerTabelPenumpang(); cetakNode(1, ptr); }
...
void tampilSemuaAntrian(const Queue &q) {
    ...
    garisPendek();
    cout << "  Total dalam antrian: " << (no - 1) << " penumpang.\n";
}
...
int main() {
    ...
    do {
        ...
        cout << "  8.  Quick Sort\n";
        cout << "  ---- Antrian & Riwayat (Linked List) ----\n";
